...
public class CandleFactory { // refatorar nome de classes = Alt+Shift+R
	...
	public Candle constroiCandleParaData(Calendar data, List<Negociacao> negocios) {
		...
		for (Negociacao negocio : negocios) {
			...
			double preco = negocio.getPreco(); // refatorar para variavel local = Alt+Shift+L
	...
	public List<Candle> constroiCandles(List<Negociacao> todasNegociacoes) {
		...
		for (Negociacao negociacao : todasNegociacoes) {
			...
			if (!negociacao.isMesmoDia(dataAtual)) {
				criaEGuardaCandle(candles, negociacoesDoDia, dataAtual); // Refatorar extrair método = Alt+Shift+M
